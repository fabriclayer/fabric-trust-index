            ...
            {service.ai_assessment ? (
              <div className="max-w-[680px]">
                ...
                <p className="font-mono text-[0.62rem] text-fabric-400 mt-3">
                  Generated by Fabric AI{service.ai_assessment_updated_at ? ` · ${new Date(service.ai_assessment_updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}` : ''}
                </p>
                {service.description && (
                  <div className="mt-4 pt-4 border-t border-fabric-100">
                    <span className="font-mono text-[0.62rem] uppercase tracking-wider text-fabric-400 font-medium">Source description</span>
                    <p className="text-[0.82rem] leading-relaxed text-fabric-400 mt-1.5">{service.description}</p>
                  </div>
                )}
              </div>
            ) : (
